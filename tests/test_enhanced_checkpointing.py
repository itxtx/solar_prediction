...
from solar_prediction.checkpointing import (
    save_checkpoint,
    load_checkpoint,
...
def test_lstm_enhanced_checkpointing():
    ...
    # Prepare test metadata
    train_cfg = {"epochs": 3, "batch_size": 32, "learning_rate": 0.001}
    metrics = {"final_val_loss": 0.7, "best_val_r2": 0.7}

    with tempfile.TemporaryDirectory() as temp_dir:
        # Test enhanced saving
        enhanced_path = Path(temp_dir) / "lstm_enhanced.pt"
        model.save(str(enhanced_path), train_cfg=train_cfg, metrics=metrics, use_enhanced=True)
...
def test_gru_enhanced_checkpointing():
    ...
    # Prepare test metadata
    train_cfg = {"epochs": 3, "batch_size": 32, "learning_rate": 0.001, "bidirectional": False}
    metrics = {"final_val_loss": 0.8, "best_val_r2": 0.6}
