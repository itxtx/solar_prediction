...
try:
    ...
    HAS_CLASSICAL_DEPS = True
except ImportError:
    HAS_CLASSICAL_DEPS = False

pytestmark = pytest.mark.skipif(
    not HAS_CLASSICAL_DEPS,
    reason="SARIMA edge tests require the classical extra",
)
...
class TestSARIMAEdgeCases:
    ...
    def create_test_dataframe(self, series_data, start_date="2023-01-01", freq="15min"):
        ...
        return pd.DataFrame({"Time": timestamps, "GHI": series_data})
    ...
    def test_all_zeros_data(self):
        ...
        try:
            ...
            assert any(
                keyword in str(e).lower() for keyword in failure_keywords
    ...
    def test_extreme_outliers(self):
        ...
        try:
            ...
            model_result = fit_sarima_model(
                train_series, order=(1, 1, 1), seasonal_order=(1, 1, 1, s)
            )
    ...
    def test_auto_arima_edge_cases(self):
        ...
        test_cases = [
            ...
            (
                "high_frequency",
                np.sin(2 * np.pi * np.arange(500) / 5) + np.random.normal(0, 0.1, 500),
            ),
        ]

        for case_name, data in test_cases:
            series = pd.Series(
                data, index=pd.date_range("2023-01-01", periods=len(data), freq="15min")
            )

            try:
                # Test auto-ARIMA with conservative parameters
                result = auto_sarima_selection(
                    series, seasonal_period=96, max_p_val=1, max_q_val=1, max_P_val=1, max_Q_val=1
                )
    ...
    def test_division_by_zero_protection(self):
        ...
        try:
            ...
            if metrics and "MAPE" in metrics:
                ...
                assert not np.isnan(metrics["MAPE"]) or np.isnan(
                    metrics["MAPE"]
                ), "MAPE handling of zeros unclear"
...
def test_end_to_end_robustness():
    ...
    challenging_datasets = {
        "mixed_issues": {
            "data": [0] * 100
            + list(np.random.normal(50, 10, 400))
            + [1000]
            + list(np.random.normal(50, 10, 499)),
            "expected_issues": ["zeros", "outliers"],
        },
        "seasonal_with_trend": {
            "data": [
                100 + 10 * np.sin(2 * np.pi * i / 96) + 0.01 * i + np.random.normal(0, 2)
                for i in range(1000)
            ],
    ...
    for dataset_name, dataset_info in challenging_datasets.items():
        ...
        df = pd.DataFrame(
            {
                "Time": pd.date_range(
                    "2023-01-01", periods=len(dataset_info["data"]), freq="15min"
                ),
