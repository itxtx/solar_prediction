...
def sample_data_path():
    ...
    return project_root / "data" / "sample" / "SolarPrediction_sample.csv"
...
def sample_data(sample_data_path):
    ...
    if "Radiation" in df.columns and "GHI" not in df.columns:
        df["GHI"] = df["Radiation"]
    if "Temperature" in df.columns and "temp" not in df.columns:
        df["temp"] = df["Temperature"]
    if "Speed" in df.columns and "wind_speed" not in df.columns:
...
def time_budget():
    ...
    return {"data_preparation": 2.0, "model_training": 10.0, "inference": 1.0, "memory_test": 5.0}
