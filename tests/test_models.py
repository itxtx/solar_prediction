...
from solar_prediction.gru import (
    WeatherGRU,
...
class TestLSTMModel:
    ...
    def test_lstm_initialization(self):
        ...
        model_params = create_model_hyperparameters_from_config(
            input_dim=5, config_override={"hidden_dim": 32, "num_layers": 1}
        )
    ...
    def test_lstm_forward_pass(self, numpy_arrays_2d, device):
        ...
        # Create model
        model_params = create_model_hyperparameters_from_config(
            input_dim=X.shape[2], config_override={"hidden_dim": 16, "num_layers": 1}
        )
        model = WeatherLSTM(model_params)
        model.to(device)
        model.eval()
    ...
    def test_lstm_prediction(self, tiny_training_data, device):
        ...
        model_params = create_model_hyperparameters_from_config(
            input_dim=data["X_train"].shape[2], config_override={"hidden_dim": 8, "num_layers": 1}
        )
...
class TestGRUModel:
    ...
    def test_gru_initialization(self):
        ...
        model_params = create_gru_model_hyperparameters_from_config(
            input_dim=5, config_override={"hidden_dim": 32, "num_layers": 1, "bidirectional": False}
        )
...
class TestModelComparisons:
    ...
    def test_model_parameter_counts(self):
        ...
        lstm_params = create_model_hyperparameters_from_config(
            input_dim=input_dim, config_override={"hidden_dim": 32, "num_layers": 2}
        )
        lstm_model = WeatherLSTM(lstm_params)
...
class TestModelEdgeCases:
    ...
    def test_model_with_minimal_data(self, device):
        ...
        # Test LSTM
        lstm_params = create_model_hyperparameters_from_config(
            input_dim=4, config_override={"hidden_dim": 4, "num_layers": 1}
        )
        training_config = create_training_config_from_config(
            config_override={"epochs": 1, "batch_size": 2}
        )
    ...
    def test_model_error_handling(self, device):
        ...
        # Create model
        model_params = create_model_hyperparameters_from_config(
            input_dim=5, config_override={"hidden_dim": 8, "num_layers": 1}
        )
        model = WeatherLSTM(model_params)
        model.to(device)
    ...
    def test_recurrent_models_explain_feature_dimension_mismatch(self, device):
        """Checkpoint/data feature mismatches should raise actionable errors."""
        X_wrong = np.random.randn(4, 6, 3).astype(np.float32)
        y = np.random.randn(4, 1).astype(np.float32)

        lstm_params = create_model_hyperparameters_from_config(
            input_dim=10, config_override={"hidden_dim": 8, "num_layers": 1}
        )
        lstm_model = WeatherLSTM(lstm_params).to(device)
        with pytest.raises(ValueError, match="checkpoint was trained with a different feature set"):
            lstm_model.evaluate(X_wrong, y, device=device, return_predictions=False)

        gru_params = create_gru_model_hyperparameters_from_config(
            input_dim=10,
            config_override={"hidden_dim": 8, "num_layers": 1, "bidirectional": False},
        )
        gru_model = WeatherGRU(gru_params).to(device)
        with pytest.raises(ValueError, match="checkpoint was trained with a different feature set"):
            gru_model.evaluate(X_wrong, y, device=device, return_predictions=False)
