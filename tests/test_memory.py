...
class TestMemoryStability:
    ...
    def test_forward_pass_memory_stability_100x(
        self, test_model_and_data, memory_tracker, time_budget
    ):
        ...
        assert (
            elapsed_time <= time_budget["memory_test"]
    ...
    def test_gru_memory_stability(self, memory_tracker, device, time_budget):
        ...
        elapsed_time = time.time() - start_time
        assert (
            elapsed_time <= time_budget["memory_test"] / 2
        ...
        if len(memory_readings) > 1:
            ...
            if memory_mean > 0:
                ...
                assert relative_std < 0.15, f"GRU memory too variable: {relative_std:.3f} > 0.15"
    ...
    def test_memory_leak_detection(self, memory_tracker, device):
        ...
        assert total_increase <= 50, f"Memory leak detected: {total_increase:.1f}MB increase > 50MB"
...
class TestMemoryEdgeCases:
    ...
    def test_large_batch_memory(self, device, memory_budget):
        ...
        # Create model
        model_params = create_model_hyperparameters_from_config(
            input_dim=20, config_override={"hidden_dim": 64, "num_layers": 2}
        )
        model = WeatherLSTM(model_params)
        model.to(device)
        model.eval()
    ...
    def test_gradient_memory_management(self, device):
        ...
        model_params = create_model_hyperparameters_from_config(
            input_dim=5, config_override={"hidden_dim": 16, "num_layers": 1}
        )
        model = WeatherLSTM(model_params)
        model.to(device)
