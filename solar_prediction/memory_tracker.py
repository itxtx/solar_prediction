...
class MemoryTracker:
    ...
    def __init__(self, device: Optional[str] = None, verbose: bool = False):
        ...
        # Let the application/notebook configure handlers. Installing a stream
    ...
    def get_memory_info(self) -> Dict[str, float]:
        ...
        return {"allocated": allocated, "cached": cached, "max_allocated": max_allocated}
    ...
    def log_memory_diff(
        self, before_name: str, after_name: str, operation_name: str = "operation"
    ) -> None:
        """
        ...
        if before_name not in self.snapshots or after_name not in self.snapshots:
            self.logger.warning(
                f"Cannot find snapshots for memory diff: {before_name}, {after_name}"
            )
            return
    ...
    def log_device_info(self) -> None:
        ...
        if self.is_cuda_available():
            ...
            if str(self.device).startswith("mps"):
                self.logger.info("Using MPS - detailed GPU memory tracking is not available")
            else:
