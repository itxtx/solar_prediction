...
def create_gru_training_config_from_config(
...
class WeatherGRU(nn.Module):
    ...
    def _validate_fit_inputs(
        ...
        if X_train.shape[0] != y_train.shape[0]:
            raise ValueError("X_train/y_train sample mismatch.")
        if X_val.shape[0] != y_val.shape[0]:
            raise ValueError("X_val/y_val sample mismatch.")
        if X_train.ndim != 3 or X_val.ndim != 3:
            raise ValueError("Input X must be 3D.")
        self._validate_sequence_input(X_train, "X_train", "GRU fit")
        self._validate_sequence_input(X_val, "X_val", "GRU fit")
        if (
            y_train.ndim != 2
            or y_val.ndim != 2
            ...
            raise ValueError(
                f"Target y must be 2D and match model output_dim={self.params.output_dim}."
            )
    ...
    def _validate_sequence_input(
        ...
        if actual_features != expected_features:
            raise ValueError(
                ...
                "a GRU checkpoint that matches the current feature_cols."
            )
    ...
    def fit(
        ...
        for epoch in range(train_config.epochs):
            ...
            memory_tracker.snapshot(
                f"epoch_{epoch}_val_start", f"start of GRU validation for epoch {epoch}"
            )
            ...
            val_preds = np.vstack(val_outputs_all)
            val_acts = np.vstack(val_targets_all)
            val_rmse = np.sqrt(mean_squared_error(val_acts, val_preds))
            ...
            val_mape_cap = (
                np.mean(
                    np.clip(
                        np.abs((val_acts - val_preds) / (np.abs(val_acts) + epsilon_mape)),
                        0,
            ...
            self.history["val_mape"].append(val_mape_cap)
            ...
            memory_tracker.log_memory_diff(
                f"epoch_{epoch}_start", f"epoch_{epoch}_end", f"GRU_epoch_{epoch}"
            )
        ...
        if transform_info:
            ...
            for t_details in reversed(transforms):
                ...
                if t_type == "log":
                    ...
                    exp_input = np.clip(
                        y_proc, -config.data.max_exp_input, config.data.max_exp_input
                    )
    ...
    def evaluate(
        ...
        self.eval()
        self.to(device)
        self._validate_sequence_input(X_test_data, "X_test_data", "GRU evaluate")
    ...
    def _ensure_batched_input(self, X: Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
        ...
        if X_t.ndim == 1:
            ...
            raise ValueError(f"Input X must be 1D, 2D, or 3D, got {X_t.ndim}D")
    ...
    def predict(
        ...
        self._validate_sequence_input(X_batch_t, "X", "GRU predict")
    ...
    def plot_training_history(
        ...
        for i, (title, keys, ylog, l_colors) in enumerate(metrics_cfg):
            ...
            for k_idx, key in enumerate(keys):
                if key in self.history and self.history[key]:
                    ...
                    ax.plot(
                        self.history["epochs"], self.history[key], color=l_colors[k_idx], label=lbl
                    )
        ...
        if (
            ...
            and len(self.history["train_loss"]) == len(self.history["val_loss"]) > 0
        ):
            ...
            ax_r.text(0.5, 0.5, "Loss data missing", transform=ax_r.transAxes, ha="center")
        ax_r.set_title("Overfitting Indicator")
        ax_r.set_xlabel("Epoch")
        ax_r.set_ylabel("Ratio")
        ax_r.legend()
        ax_r.grid(True)
        if len(axes) > 7 and fig.axes[-1] == axes[7]:
            fig.delaxes(axes[7])
        plt.suptitle("GRU Model Training Metrics", fontsize=16, fontweight="bold")
            ...
            if target_scaler and transform_info:
                ...
                for batch_start in range(0, mc_samples, inverse_batch_size):
                    ...
                    for i in range(batch_start, batch_end):
                        ...
                        inverted_s = self._inverse_transform_target(
                            curr_batch_s, target_scaler, transform_info, scalers_dict
                        )
    ...
    def plot_prediction_with_uncertainty(
        ...
        uncertainty_res = self.predict_with_uncertainty(
            X_for_pred.cpu().numpy(),
            mc_samples,
            device,
            target_scaler,
            curr_tf_info,
        ...
        plot_idxs = (
            np.arange(min(max_samples_to_plot, num_avail_s))
            if indices is None
            else [i for i in np.array(indices) if i < num_avail_s]
        )
        ...
        if y_true is not None:
            ...
            if target_scaler and curr_tf_info:
                y_true_orig = self._inverse_transform_target(
                    y_true_arr.reshape(-1, self.params.output_dim),
                    target_scaler,
                    curr_tf_info,
                    scalers_dict,
                ).flatten()
        ...
        for i, s_idx_batch in enumerate(plot_idxs):
            ax = axs[i, 0]
            mean = uncertainty_res["mean"][s_idx_batch, 0]
            low = uncertainty_res["lower_ci"][s_idx_batch, 0]
            upp = uncertainty_res["upper_ci"][s_idx_batch, 0]
            mc_samps_item = uncertainty_res["samples"][:, s_idx_batch, 0]
            for j in range(min(10, mc_samples)):
                ax.plot([0], [mc_samps_item[j]], "o", alpha=0.15, c="gray", ms=3)
            ax.errorbar(
                [0],
                [mean],
                yerr=[[mean - low], [upp - mean]],
                fmt="o",
                c="orange",
                ecolor="sandybrown",
                capsize=5,
            ...
            if y_true_orig is not None and s_idx_batch < len(y_true_orig):
                act = y_true_orig[s_idx_batch]
                ax.plot([0], [act], "ro", label="Actual")
                err = abs(mean - act)
                in_ci = low <= act <= upp
            ...
            min_y_p = min(
                [low]
                + (
                    [y_true_orig[s_idx_batch]]
                    if y_true_orig is not None and s_idx_batch < len(y_true_orig)
                    else []
                )
            )
            min_y_p = min_y_p if min_y_p is not None else 0
            max_y_p = max(
                [upp]
                + (
                    [y_true_orig[s_idx_batch]]
            ...
            max_y_p = max_y_p if max_y_p is not None else 1
            pad = 0.2 * (max_y_p - min_y_p) if (max_y_p - min_y_p) > 1e-6 else 1.0
            config = get_config()
            ax.set_ylim([max(config.data.min_radiation_clip, min_y_p - pad), max_y_p + pad])
            ax.set_xticks([])
            ax.set_ylabel("GHI (Original)", fontsize=9)
            if i == 0:
                ax.legend(fontsize=8, loc="best")
        plt.suptitle(
            f"GRU Preds w {int((1-alpha)*100)}% CI (Original Scale)", fontsize=14, fontweight="bold"
        )
