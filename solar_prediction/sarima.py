...
def plot_forecast_comparison(
    ...
    ax1.plot(
        train_data.index[-context_points:],
        train_data.iloc[-context_points:],
        label="Training (last portion)",
        color="gray",
        alpha=0.6,
        linewidth=1,
    )
    ...
    for i, (model_name, pred) in enumerate(predictions_dict.items()):
        ax1.plot(pred.index, pred, label=model_name, color=colors[i], linewidth=1.5, alpha=0.8)
    ...
    ax2.plot(
        test_data.index[:zoom_points],
        test_data.iloc[:zoom_points],
        label="Actual",
        color="black",
        linewidth=2,
        marker="o",
    ...
    for i, (model_name, pred) in enumerate(predictions_dict.items()):
        ax2.plot(
            pred.index[:zoom_points],
            pred.iloc[:zoom_points],
            label=model_name,
            color=colors[i],
            linewidth=2,
            alpha=0.8,
    ...
    for i, (model_name, pred) in enumerate(predictions_dict.items()):
        ...
        ax3.plot(
            residuals.index, residuals, label=f"{model_name} Residuals", color=colors[i], alpha=0.7
        )
    ...
    for i, (model_name, pred) in enumerate(predictions_dict.items()):
        residuals = test_data - pred
        ax4.hist(
            residuals, bins=20, alpha=0.6, label=f"{model_name}", color=colors[i], density=True
        )
...
def plot_error_analysis(test_data, predictions_dict, figsize=(16, 12)):
    ...
    for i, (model_name, pred) in enumerate(predictions_dict.items()):
        ax3.scatter(test_data, pred, alpha=0.6, label=model_name, color=colors[i], s=20)
    ...
    for i, (model_name, abs_err) in enumerate(abs_errors.items()):
        ...
        ax4.plot(abs_err.index, cumulative_error, label=model_name, color=colors[i], linewidth=2)
    ...
    try:
        ...
        if metrics_summary is not None:
            ...
            ax6.text(
                0.5, 0.5, "No metrics available", transform=ax6.transAxes, ha="center", va="center"
            )
...
def create_performance_summary_plots(metrics_df, figsize=(16, 10)):
    ...
    for i, model in enumerate(models):
        ax5.annotate(
            model,
            (metrics_sorted.iloc[i]["MAE"], metrics_sorted.iloc[i]["RMSE"]),
            xytext=(5, 5),
    ...
    try:
        ...
        bars6 = ax6.barh(
            model_names, relative_improvement, color=colors[1 : len(model_names) + 1], alpha=0.8
        )
