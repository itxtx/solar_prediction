...
def floor_prob(
    prob: Union[float, np.ndarray], min_prob: float = 1e-300
...
class SolarTDMC:
    ...
    def _preprocess_data(
        self, X: np.ndarray, timestamps: Optional[Union[np.ndarray, pd.Series]] = None
        ...
        if timestamps is not None:
            if isinstance(
                timestamps.iloc[0] if isinstance(timestamps, pd.Series) else timestamps[0],
    ...
    def fit(
        ...
        if timestamps is not None and len(X) != len(timestamps):
            raise ValueError(
                f"X and timestamps must have the same length. Got X: {len(X)}, timestamps: {len(timestamps)}"
            )
        if X.shape[1] != self.n_emissions:
            raise ValueError(
                f"X has {X.shape[1]} features, but model expects {self.n_emissions} emissions."
            )
        ...
        if logger:
            logger.info(
                f"Starting TDMC fit with {self.n_states} states, {self.n_emissions} emissions"
            )
    ...
    def _initialize_parameters(self, X_scaled: np.ndarray, time_indices: np.ndarray):
        ...
        kmeans = KMeans(
            ...
            random_state=tdmc_config.random_state,
            n_init=tdmc_config.kmeans_n_init,
        )
        ...
        for s in range(self.n_states):
            ...
            if len(state_data) > 1:  # Need at least 2 samples for covariance
                ...
                if min_eig <= 0:  # Ensure positive definiteness
                    cov += (-min_eig + tdmc_config.min_eigenvalue_threshold) * np.eye(
                        self.n_emissions
                    )  # Add offset to make eigenvalues positive
                self.emission_covars[s] = cov + tdmc_config.covariance_regularization * np.eye(
                    self.n_emissions
                )  # Regularization
            elif len(state_data) == 1:
                self.emission_means[s] = state_data[0]
                self.emission_covars[s] = (
                    np.eye(self.n_emissions) * 1e-4
                ...
                if np.any(np.isnan(self.emission_covars[s])) or self.emission_covars[s].shape != (
                    self.n_emissions,
                    self.n_emissions,
                ):
        ...
        self.transitions = (
            np.ones((self.time_slices, self.n_states, self.n_states)) * 1e-6
        )  # Smoothing prior
        for t in range(self.time_slices):
            ...
            self.transitions[t] = self.transitions[t] / np.maximum(
                row_sums, 1e-300
        ...
        unique_first_time_indices_mask = np.concatenate(
            ([True], time_indices[1:] != time_indices[:-1])
        )
    ...
    def _forward_backward(
        ...
        for s in range(self.n_states):
            try:
                ...
                mvn = multivariate_normal(
                    mean=self.emission_means[s], cov=cov, allow_singular=False
                )
        ...
        if logger:
            logger.debug(
                f"Emission probabilities - min: {np.min(emission_probs):.2e}, max: {np.max(emission_probs):.2e}"
            )
        ...
        alpha[0] = safe_normalise(
            alpha[0].reshape(1, -1), min_val=tdmc_config.min_probability
        ).flatten()

        for t in range(1, n_samples):
            current_time_slice_transitions = self.transitions[
                time_indices[t - 1]
            ]  # Transitions from t-1 to t
            ...
            alpha[t] = safe_normalise(
                alpha[t].reshape(1, -1), min_val=tdmc_config.min_probability
            ).flatten()

        # Backward pass with safe normalization
        beta[-1] = 1.0
        for t in range(n_samples - 2, -1, -1):
            next_time_slice_transitions = self.transitions[
                time_indices[t]
        ...
        if logger:
            logger.debug(
                f"Forward-backward complete - scale range: [{np.min(scale):.2e}, {np.max(scale):.2e}]"
            )

        return alpha, beta, scale, emission_probs

    def _baum_welch_update(
        self, X_scaled: np.ndarray, time_indices: np.ndarray, max_iter: int, tol: float
    ) -> float:
        ...
        for iteration in range(max_iter):
            alpha, beta, scale, emission_probs = self._forward_backward(
                X_scaled, time_indices, logger
            )
            ...
            if abs(current_log_likelihood - prev_log_likelihood) < tol and iteration > 0:
                ...
                if logger:
                    logger.info(
                        f"Converged at iteration {iteration+1}, Log-Likelihood: {log_likelihood:.4f}"
                    )
                break
            ...
            if logger and (iteration % tdmc_config.log_likelihood_every_n_iter == 0):
                logger.info(
                    f"Iteration {iteration+1}/{max_iter}, Log-Likelihood: {log_likelihood:.4f}"
                )

            n_samples = len(X_scaled)
            gamma = alpha * beta
            gamma = gamma / np.maximum(
                np.sum(gamma, axis=1, keepdims=True), 1e-300
            )  # Normalize gamma

            # Update initial state distribution with safe normalization
            self.initial_probs = safe_normalise(
                gamma[0].reshape(1, -1), min_val=tdmc_config.min_probability
            ).flatten()
            ...
            for ts in range(self.time_slices):
                ...
                for i in range(self.n_states):
                    ...
                    self.transitions[ts, i, :] = safe_normalise(
                        self.transitions[ts, i, :].reshape(1, -1), min_val=min_prob
                    ).flatten()
            ...
            for s in range(self.n_states):
                ...
                gamma_s_sum = np.maximum(
                    gamma_s_sum, tdmc_config.min_probability
                )  # Avoid division by zero
                ...
                if not np.any(np.isnan(cov_s)):
                    ...
                    if logger:
                        ...
                        logger.debug(
                            f"Updated covariance for state {s}, min eigenvalue: {min_eig:.2e}"
                        )
    ...
    def predict_states(
        ...
        for s in range(self.n_states):
            try:
                ...
                precision = np.diag(1.0 / np.maximum(np.diag(self.emission_covars[s]), 1e-9))
                emission_probs[:, s] = np.exp(
                    -0.5 * np.sum((X_scaled - self.emission_means[s]) ** 2 @ precision, axis=1)
                )

        # Apply probability floor for numerical stability
        emission_probs = floor_prob(
            emission_probs, tdmc_config.min_probability
        )  # Avoid log(0) later

        if logger:
            logger.debug(
                f"Emission probabilities computed - min: {np.min(emission_probs):.2e}, max: {np.max(emission_probs):.2e}"
            )
        ...
        # Recursion step
        for t in range(1, n_samples):
            current_time_slice = time_indices[
                t - 1
            ]  # Transition from t-1 to t uses A[time_indices[t-1]]
        ...
        if logger:
            ...
            logger.debug(
                f"Predicted state distribution: {dict(zip(self.state_names, state_counts))}"
            )
    ...
    def forecast(
        self,
        X_last: np.ndarray,
        timestamp_last_hour: int,
        forecast_horizon: int,
        weather_forecasts: Optional[np.ndarray] = None,
        external_forecast_weight: float = 0.3,
        ...
        for s in range(self.n_states):
            try:
                ...
                if np.min(np.linalg.eigvalsh(cov)) <= 1e-9:
                    cov += 1e-6 * np.eye(self.n_emissions)
                mvn = multivariate_normal(
                    mean=self.emission_means[s], cov=cov, allow_singular=False
                )
                ...
                precision = np.diag(1.0 / np.maximum(np.diag(self.emission_covars[s]), 1e-9))
                current_emission_probs[s] = np.exp(
                    -0.5
                    * np.sum((X_last_scaled.reshape(-1) - self.emission_means[s]) ** 2 @ precision)
                )
        ...
        future_time_indices = [
            (timestamp_last_hour + i + 1) % self.time_slices for i in range(forecast_horizon)
        ]

        for step in range(forecast_horizon):
            time_idx_for_transition = (
                future_time_indices[step - 1] if step > 0 else timestamp_last_hour
            )
            ...
            for s in range(self.n_states):
                expected_var_within_states += state_distribution[s] * np.diag(
                    self.emission_covars[s]
                )
                var_of_expected_values += (
                    state_distribution[s]
                    * (self.emission_means[s] - step_forecast_mean_scaled) ** 2
                )

            step_forecast_var_scaled = expected_var_within_states + var_of_expected_values
            step_forecast_std_scaled = np.sqrt(
                np.maximum(step_forecast_var_scaled, 1e-9)
            )  # Avoid sqrt of negative/small
        ...
        if weather_forecasts is not None:
            for step in range(min(forecast_horizon, len(weather_forecasts))):
                ...
                if len(external_pred) == self.n_emissions:
                    forecasts_orig[step] = (1 - external_forecast_weight) * forecasts_orig[
                        step
    ...
    def get_state_characteristics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for s_idx in range(self.n_states):
            ...
            scaler_scales = (
                self.scaler.scale_ if self.scaler.scale_ is not None else np.ones(self.n_emissions)
            )
            ...
            state_info[self.state_names[s_idx]] = {
                ...
                "most_likely_next_states": {
                    f"hour_{t}": sorted(
                        [
                            (self.state_names[next_s], self.transitions[t, s_idx, next_s])
    ...
    def plot_state_transitions(
        self, time_slice: int = 12, ax: Optional[plt.Axes] = None
    ) -> plt.Figure:
        if not self.trained:
            raise ValueError("Model not trained yet.")
        ...
        ax_plot.set_title(
            f"Transition Probabilities at Time Slice {time_slice} (Hour {time_slice})"
        )
    ...
    def plot_emissions_by_state(
        self, standardized: bool = False, ax: Optional[plt.Axes] = None
    ) -> plt.Figure:
        if not self.trained:
            raise ValueError("Model not trained yet.")
        ...
        if not standardized:
            ...
            for s_idx in range(self.n_states):
                covars_to_plot[s_idx] = self.emission_covars[s_idx] * np.outer(
        ...
        if self.n_emissions == 1:
            ...
            for s in range(self.n_states):
                try:
                    ...
                    if cov <= 1e-9:
                        cov = 1e-9  # ensure positive for pdf
            ...
            for s in range(self.n_states):
                try:
                    ...
                    if np.min(np.linalg.eigvalsh(cov)) <= 1e-9:
                        cov += 1e-6 * np.eye(self.n_emissions)  # Ensure positive definite
                    ...
                    ax_plot.contour(
                        ...
                        levels=3,
                        alpha=0.7,
                        colors=[plt.cm.viridis(s / max(1, self.n_states - 1))],
                    )  # Use distinct colors
                    ax_plot.scatter(
                        means_to_plot[s, 0],
                        means_to_plot[s, 1],
                        ...
                        s=100,
                        label=(
                            self.state_names[s] if not ax_plot.legend_ else None
    ...
    def save_model(
        self, filepath: str, hp=None, train_cfg=None, history=None, metrics=None, use_enhanced=True
    ):
    ...
    @classmethod
    def load_model(
        cls, filepath: str, map_location: Optional[str] = None, strict: bool = False
    ) -> "SolarTDMC":
        ...
        if file_path.suffix == ".pt":
            ...
            try:
                ...
                print(
                    f"Loaded TDMC model from enhanced checkpoint (version {metadata.get('version')})"
                )
                return model
        ...
        try:
            ...
            )  # allow_pickle for state_names if they are objects
        ...
        model.state_names = (
            [str(sn) for sn in data["state_names"]]
            if "state_names" in data
            else [f"State_{i}" for i in range(model.n_states)]
        )
        ...
        if data["scaler_mean_"] is not None and data["scaler_scale_"] is not None:
            ...
            model.scaler.n_features_in_ = model.n_emissions
            if model.scaler.mean_ is not None:
                model.scaler.n_samples_seen_ = 1  # Dummy value to indicate it's fitted
