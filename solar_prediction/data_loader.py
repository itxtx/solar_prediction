...
class ChunkedDataLoader:
    ...
    def load_chunks(self, file_path: str, **read_csv_kwargs) -> Iterator[pd.DataFrame]:
        ...
        self.logger.info(
            f"Starting chunked reading of {file_path} with chunksize={self.config.chunksize}"
        )
        ...
        self.logger.info(
            f"Completed chunked reading: {chunk_count} chunks, {total_rows} total rows"
        )
    ...
    def _apply_basic_feature_engineering(self, chunk: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Timestamp" in processed_chunk.columns:
            try:
                processed_chunk["Timestamp"] = pd.to_datetime(
                    processed_chunk["Timestamp"], cache=True
                )
                ...
                processed_chunk["DayOfYearSin"] = np.sin(
                    2 * np.pi * processed_chunk["DayOfYear"] / 365
                )
                processed_chunk["DayOfYearCos"] = np.cos(
                    2 * np.pi * processed_chunk["DayOfYear"] / 365
                )
        ...
        if "Temperature" in processed_chunk.columns and "Humidity" in processed_chunk.columns:
            processed_chunk["TempHumidityInteraction"] = (
                processed_chunk["Temperature"] * processed_chunk["Humidity"]
            )

        return processed_chunk

    def load_and_combine_chunks(
        self, file_path: str, max_chunks: Optional[int] = None, **read_csv_kwargs
    ) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Combined {len(chunks)} chunks into DataFrame with {len(combined_df)} rows"
        )
...
def prepare_weather_data_chunked(
    ...
    return prepare_weather_data(
        df, input_cfg, transform_cfg, feature_cfg, scaling_cfg, sequence_cfg
    )
