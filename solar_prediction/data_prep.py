...
from .config import (
    get_config,
    DataInputConfig,
    DataTransformationConfig as TransformationConfig,
...
def _copy_input_config_with_target(cfg: DataInputConfig, target_col: str) -> DataInputConfig:
    """Return a copy of the input config with an updated target column."""
    if hasattr(cfg, "model_copy"):
        return cfg.model_copy(update={"target_col_original_name": target_col})
    return cfg.copy(update={"target_col_original_name": target_col})


def _resolve_input_config(df_input: pd.DataFrame, cfg: DataInputConfig) -> DataInputConfig:
    """Resolve common target aliases so both portfolio datasets are first-class inputs."""
    if cfg.target_col_original_name in df_input.columns:
        return cfg
    ...
    for alias in target_aliases:
        if alias in df_input.columns:
            ...
            return _copy_input_config_with_target(cfg, alias)
...
def _compute_sequence_split_metadata(n_rows: int, sequence_cfg: SequenceConfig) -> Dict[str, Any]:
    ...
    if n_train_val <= 0:
        raise ValueError("No train/validation sequences remain after test split.")

    n_val = (
        int(math.ceil(sequence_cfg.val_size_from_train_val * n_train_val))
        if sequence_cfg.val_size_from_train_val > 0 and n_train_val > 1
        else 0
    )
    ...
    return {
        ...
        "train_sequence_range": (0, n_train),
        "val_sequence_range": (n_train, n_train + n_val),
        "test_sequence_range": (n_train + n_val, n_sequences),
        "feature_fit_row_range": (0, feature_fit_end),
        "target_fit_row_range": (target_fit_start, target_fit_end),
        "feature_fit_indices": np.arange(0, feature_fit_end, dtype=int),
        "target_fit_indices": np.arange(target_fit_start, target_fit_end, dtype=int),
    }
...
def _initial_df_setup(
    ...
    rename_map = {
        ...
        "Speed": STD_WINDSPEED_COL,  # Kaggle SolarPrediction dataset
    ...
    standardized_target_name = (
        STD_RADIATION_COL
        if cfg.target_col_original_name == cfg.common_ghi_col
        else (
            STD_TEMP_COL
            if cfg.target_col_original_name == cfg.common_temp_col
            else cfg.target_col_original_name
        )
    )

    if (
        cfg.target_col_original_name not in rename_map
        or rename_map[cfg.target_col_original_name] != standardized_target_name
    ):
    ...
    if final_target_col_name not in df.columns:  # Should not happen if logic is correct
        raise ValueError(
            f"Target column '{final_target_col_name}' (from '{cfg.target_col_original_name}') not found after renaming."
        )

    # Time sorting with optimized datetime parsing. Prefer UNIX timestamps when
    # present because the Kaggle sample has a clock-only "Time" column.
...
def _engineer_time_features(
    ...
    if input_cfg.sunrise_col in df.columns and input_cfg.sunset_col in df.columns:
        ...
        sunrise_minutes = pd.Series(
            _parse_time_to_minutes_vectorized(df[input_cfg.sunrise_col]), index=df.index
        )
        sunset_minutes = pd.Series(
            _parse_time_to_minutes_vectorized(df[input_cfg.sunset_col]), index=df.index
        )
        ...
        if sunrise_minutes.notna().all() and sunset_minutes.notna().all():
            ...
            df[STD_IS_DAYLIGHT] = (
                (current_time_minutes >= sunrise_minutes) & (current_time_minutes <= sunset_minutes)
            ).astype(float)
            ...
            time_since_sunrise = np.where(
                negative_mask, time_since_sunrise + minutes_in_day, time_since_sunrise
            )
            ...
            df[STD_TIME_UNTIL_SUNSET] = np.where(
                time_until_sunset < 0, time_until_sunset + minutes_in_day, time_until_sunset
            )
    ...
    # Fallback to raw columns if they exist and parsed ones are missing
        ...
        df[STD_DAYLIGHT_MINUTES] = (
            pd.to_numeric(df[input_cfg.daylength_col_raw], errors="coerce") * 60
        )  # Assuming dayLength is in hours
    ...
    if (
        input_cfg.sunlight_time_daylength_ratio_raw in df.columns
        and STD_DAYLIGHT_POSITION not in df.columns
    ):
        df[STD_DAYLIGHT_POSITION] = pd.to_numeric(
            df[input_cfg.sunlight_time_daylength_ratio_raw], errors="coerce"
        ).clip(0, 1)
...
def _apply_target_transformations(
    ...
    fit_index = df.index[fit_indices]
    ...
    if is_radiation_target and cfg.use_piecewise_transform_target:
        ...
        transformed[moderate_mask] = val_at_night_thresh_end + cfg.piecewise_moderate_slope * (
            radiation_values[moderate_mask] - cfg.piecewise_night_threshold
        )

        high_mask = radiation_values >= cfg.piecewise_moderate_threshold
        # Value at the end of moderate_mask
        val_at_moderate_thresh_end = val_at_night_thresh_end + cfg.piecewise_moderate_slope * (
            cfg.piecewise_moderate_threshold - cfg.piecewise_night_threshold - 1e-6
        )
        transformed[high_mask] = val_at_moderate_thresh_end + cfg.piecewise_high_slope * (
            radiation_values[high_mask] - cfg.piecewise_moderate_threshold
        )
        ...
        applied_transforms_log.append(
            {
                "type": "piecewise",
                "original_col": current_target_col,
    ...
    if cfg.use_power_transform:
        ...
        if not np.all(all_nan_mask):
            transformed_values[~all_nan_mask] = power_transformer.transform(
                values_to_transform[~all_nan_mask]
            )
        ...
        applied_transforms_log.append(
            {
                ...
                "applied": True,
                "lambda": (
                    power_transformer.lambdas_[0]
                    if power_transformer.lambdas_ is not None
                    else None
                ),
...
def _engineer_other_domain_features(
    df: pd.DataFrame,
    target_col_after_transforms: str,
    feature_cfg: FeatureEngineeringConfig,
    standardized_target_col_name: str,
    ...
    col_for_low_indicator_ref = (
        target_col_after_transforms  # This is after structural, before scaling.
    )
    ...
    if feature_cfg.create_low_target_indicator and col_for_low_indicator_ref in df.columns:
        try:
            ...
            if (
                pd.api.types.is_numeric_dtype(df[col_for_low_indicator_ref])
                and df[col_for_low_indicator_ref].nunique() > 1
            ):
                fit_index = df.index[fit_indices]
                low_threshold = df.loc[fit_index, col_for_low_indicator_ref].quantile(
                    feature_cfg.low_target_indicator_quantile
                )
...
def _get_base_feature_set(df_columns: pd.Index, feature_cfg: FeatureEngineeringConfig) -> List[str]:
    ...
    if feature_cfg.feature_selection_mode == "minimal":
        ...
        potential_all_features = [
            ...
            STD_RAIN_COL,
            STD_SNOW_COL,
            STD_WEATHER_TYPE_COL,
            # Add all engineered time features that are always generated
            STD_HOUR_OF_DAY,
            STD_MONTH,
            STD_TIME_MINUTES_SIN,
            STD_TIME_MINUTES_COS,
            STD_DAYLIGHT_MINUTES,
            STD_IS_DAYLIGHT,
            STD_DAYLIGHT_POSITION,
            STD_SOLAR_ELEVATION,
            STD_TIME_SINCE_SUNRISE,
            STD_TIME_UNTIL_SUNSET,
        ]
...
def _scale_data(
    ...
    feature_fit_indices: np.ndarray,
    ...
    for col in feature_cols:
        ...
        feature_fit_values = df[col].ffill().bfill().iloc[feature_fit_indices].values.reshape(-1, 1)
        scaler = FeatureScalerClass()
        scaler.fit(feature_fit_values)
        scaled_df_data[col] = scaler.transform(feature_values).flatten()
        scalers[col] = scaler

    # Scale target
    if target_col_to_scale not in df.columns:
        raise ValueError(
            f"Target column for scaling '{target_col_to_scale}' not found in DataFrame."
        )
    if df[target_col_to_scale].isnull().all():
        raise ValueError(
            f"All values in target column '{target_col_to_scale}' are NaN before scaling."
        )
    ...
    yj_was_applied = any(
        t["type"] == "yeo-johnson" for t in applied_target_transforms_info if t["applied"]
    )
    ...
    target_fit_values = (
        df[target_col_to_scale].ffill().bfill().iloc[target_fit_indices].values.reshape(-1, 1)
    )
    target_scaler_instance = TargetScalerClass()

    scaled_target_col_name = f"{target_col_to_scale}_scaled"
    target_scaler_instance.fit(target_fit_values)
    scaled_df_data[scaled_target_col_name] = target_scaler_instance.transform(
        target_values
    ).flatten()
    ...
    for transform_detail in applied_target_transforms_info:
        if (
            transform_detail.get("type") == "yeo-johnson"
            and "power_transformer_object" in transform_detail
        ):
            scalers["power_transformer_object_for_target"] = transform_detail[
                "power_transformer_object"
            ]
            break
...
def _create_sequences_and_split(
    ...
    if not (feature_cols and target_col_scaled in scaled_df.columns):
        raise ValueError(
            "Feature columns list is empty or scaled target column not in DataFrame for sequencing."
        )
    ...
    if sequence_length <= 0:
        ...
        empty_y_shape = (0, 1)
        return (
            np.empty(empty_X_shape),
            np.empty(empty_X_shape),
            np.empty(empty_X_shape),
            np.empty(empty_y_shape),
            np.empty(empty_y_shape),
            np.empty(empty_y_shape),
        )
    ...
    if len(X_train_val) > 1 and sequence_cfg.val_size_from_train_val > 0:
        ...
        val_X_shape = (
            (0, X_train.shape[1], X_train.shape[2])
            if X_train.ndim == 3 and X_train.shape[0] > 0
            else (0, 0, 0)
        )
        val_y_shape = (
            (0, y_train.shape[1]) if y_train.ndim == 2 and y_train.shape[0] > 0 else (0, 1)
        )
