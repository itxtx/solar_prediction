...
class DataProcessingConfig(BaseModel):
    ...
    outlier_percentile_lower: float = Field(
        1.0, description="Lower percentile for outlier clipping"
    )
    outlier_percentile_upper: float = Field(
        99.0, description="Upper percentile for outlier clipping"
    )
...
class DataTransformationConfig(BaseModel):
    ...
    use_piecewise_transform_target: bool = Field(
        False, description="Apply piecewise transformation for GHI"
    )

    # Target preprocessing
    min_target_threshold_initial: Optional[float] = Field(
        None, description="Initial floor for original target"
    )
    clip_original_target_before_power_transform: bool = Field(
        False, description="Clip target before power transform"
    )
    original_target_clip_lower_percentile: float = Field(
        5.0, description="Lower percentile for target clipping"
    )
    original_target_clip_upper_percentile: float = Field(
        95.0, description="Upper percentile for target clipping"
    )
    ...
    min_radiation_floor_before_power_transform: float = Field(
        0.0, description="Floor for radiation before power transform"
    )

    # Piecewise transformation constants
    piecewise_night_threshold: float = Field(
        10.0, description="Night threshold for piecewise transform"
    )
    piecewise_moderate_threshold: float = Field(
        200.0, description="Moderate threshold for piecewise transform"
    )
...
class FeatureEngineeringConfig(BaseModel):
    ...
    create_low_target_indicator: bool = Field(
        True, description="Create low target indicator feature"
    )
    low_target_indicator_quantile: float = Field(
        0.1, description="Quantile for low target indicator"
    )

    # Feature selection
    feature_selection_mode: str = Field(
        "all", description="Feature selection mode: 'all', 'basic', 'minimal'"
    )
...
class DataInputConfig(BaseModel):
    ...
    sunlight_time_daylength_ratio_raw: str = Field(
        "SunlightTime/daylength", description="Sunlight ratio column"
    )
...
class ScalingConfig(BaseModel):
    ...
    standardize_features: bool = Field(
        True, description="Use StandardScaler (True) or MinMaxScaler (False)"
    )
...
class PathsConfig(BaseModel):
    ...
    sample_dataset_filename: str = Field(
        "SolarPrediction_sample.csv", description="Sample dataset filename"
    )
...
class PlottingConfig(BaseModel):
    ...
    outlier_std_threshold: float = Field(
        2.0, description="Standard deviation threshold for outlier highlighting"
    )
...
class SolarPredictionConfig(BaseModel):
    ...
    description: str = Field(
        "Solar Prediction Project Configuration", description="Configuration description"
    )

    # Configuration sections
    data: DataProcessingConfig = Field(default_factory=DataProcessingConfig)
    input: DataInputConfig = Field(
        default_factory=lambda: DataInputConfig(target_col_original_name="GHI")
    )
