...
class CombinedLoss(nn.Module):
    ...
    def __init__(
        ...
        clip_mape_percentage: Optional[float] = None,
        ...
        self.clip_mape_fraction = (
            clip_mape_percentage
            if clip_mape_percentage is not None
            else lstm_config.mape_clip_percentage
        ) / 100.0
    ...
    def _value_aware_combined_loss_calc(
        self, y_pred: torch.Tensor, y_true: torch.Tensor, value_multiplier: float
    ) -> torch.Tensor:
    ...
    def forward(
        self, y_pred: torch.Tensor, y_true: torch.Tensor, value_multiplier: Optional[float] = 0.01
    ) -> torch.Tensor:
...
class WeatherLSTM(nn.Module):
    ...
    def _validate_fit_inputs(
        ...
        if X_train.ndim != 3 or X_val.ndim != 3:
            raise ValueError(
                "Input features (X_train, X_val) must be 3-dimensional (samples, sequence_length, num_features)."
            )
        self._validate_sequence_input(X_train, "X_train", "LSTM fit")
        self._validate_sequence_input(X_val, "X_val", "LSTM fit")
        if (
            y_train.ndim != 2
            or y_val.ndim != 2
            or y_train.shape[-1] != self.params.output_dim
            or y_val.shape[-1] != self.params.output_dim
        ):
            raise ValueError(
                f"Target variables (y_train, y_val) must be 2-dimensional (samples, output_dim) and match model output_dim={self.params.output_dim}."
            )

    def _validate_sequence_input(
        self, X: Union[np.ndarray, torch.Tensor], name: str, context: str
    ) -> None:
        """Validate recurrent input shape before PyTorch raises a low-level RNN error."""
        if X.ndim != 3:
            raise ValueError(
                f"{context} expected {name} to be 3D "
                "(samples, sequence_length, num_features), "
                f"got {X.ndim}D."
            )

        actual_features = X.shape[-1]
        expected_features = self.params.input_dim
        if actual_features != expected_features:
            raise ValueError(
                f"{context} expected {name} with {expected_features} features, "
                f"got {actual_features}. This usually means the checkpoint was trained "
                "with a different feature set than the current data. Re-run preprocessing "
                "with the same feature configuration used for the checkpoint, or retrain/load "
                "an LSTM checkpoint that matches the current feature_cols."
            )
    ...
    def fit(
        ...
        if train_config.scheduler_type.lower() == "plateau":
            ...
            T_max = (
                train_config.T_max_cosine
                if train_config.T_max_cosine is not None
                else train_config.epochs
            )
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=T_max, eta_min=train_config.min_lr
            )
        else:
            raise ValueError(f"Unknown scheduler type: {train_config.scheduler_type}")
        ...
        for epoch in range(train_config.epochs):
            ...
            for inputs, targets in train_loader:
                ...
                with torch.amp.autocast("cuda", enabled=use_amp):
                    ...
                    if isinstance(criterion, CombinedLoss) and criterion.loss_mode == "value_aware":
                        loss = criterion(
                            outputs, targets, value_multiplier=train_config.value_multiplier
                        )
                    else:
                ...
                if use_amp:
                    ...
                    if train_config.clip_grad_norm is not None and train_config.clip_grad_norm > 0:
                        ...
                        torch.nn.utils.clip_grad_norm_(
                            self.parameters(), train_config.clip_grad_norm
                        )
            ...
            memory_tracker.snapshot(
                f"epoch_{epoch}_val_start", f"start of validation for epoch {epoch}"
            )
            ...
            val_mape_capped = (
                np.mean(
                    np.clip(
                        np.abs(
                            (val_actuals - val_predictions) / (np.abs(val_actuals) + epsilon_mape)
                        ),
                        0,
                        config.evaluation.mape_clip_value,
                    )
                )
                * 100
            )
            ...
            self.history["val_mape"].append(val_mape_capped)
            ...
            memory_tracker.log_memory_diff(
                f"epoch_{epoch}_start", f"epoch_{epoch}_end", f"epoch_{epoch}"
            )
    ...
    def _apply_inverse_scaling(
        self, y: np.ndarray, target_scaler: Any, transform_info: Dict
    ) -> np.ndarray:
        ...
        if target_scaler is not None:
            if hasattr(target_scaler, "n_features_in_") and target_scaler.n_features_in_ > 1:
                target_col_name = transform_info.get(
                    "target_col_transformed_final", transform_info.get("target_col_original")
                )
                target_idx = -1
                if (
                    hasattr(target_scaler, "feature_names_in_")
                    and target_scaler.feature_names_in_ is not None
                ):
                    try:
    ...
    def _apply_inverse_structural_transforms(
        self, y: np.ndarray, transform_info: Dict, scalers_dict: Optional[Dict] = None
    ) -> np.ndarray:
        """Handles inverse structural transformations (log, piecewise, Yeo-Johnson)."""
        ...
        if transform_info:
            ...
            for t_details in reversed(transforms):
                ...
                if t_type == "log":
                    ...
                    exp_input = np.clip(
                        y_processed, -config.data.max_exp_input, config.data.max_exp_input
                    )
                    ...
                    y_original = np.empty_like(y_processed, dtype=float)
                    night_mask = y_processed <= night_end
                    moderate_mask = (y_processed > night_end) & (y_processed <= moderate_end)
                    high_mask = y_processed > moderate_end
                    y_original[night_mask] = np.expm1(y_processed[night_mask])
                    y_original[moderate_mask] = (
                        night_thresh + (y_processed[moderate_mask] - night_end) / moderate_slope
                    )
                    y_original[high_mask] = (
                        moderate_thresh + (y_processed[high_mask] - moderate_end) / high_slope
                    )
                    y_processed = y_original
                elif t_type == "yeo-johnson":
                    lambda_val = t_details.get("lambda")
                    pt_obj = (
                        scalers_dict.get("power_transformer_object_for_target")
                        if scalers_dict
                        else None
                    )
                    if pt_obj and lambda_val is not None:
                        y_processed = pt_obj.inverse_transform(
    ...
    def _inverse_transform_target(
        ...
        y_untransformed_1d = self._apply_inverse_structural_transforms(
            y_unscaled_1d, transform_info, scalers_dict
        )
    ...
    def evaluate(
        ...
        Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Dict
    ]:
        """
        ...
        self.eval()
        self.to(device)
        self._validate_sequence_input(X_test_data, "X_test_data", "LSTM evaluate")
        ...
        with torch.inference_mode():
            for inputs, targets in test_loader:
                ...
                abs_percentage_error = np.abs(
                    (actual_batch - pred_batch)
                    / (np.abs(actual_batch) + config.evaluation.mape_epsilon)
                )
                sum_percentage_error += np.sum(
                    np.clip(abs_percentage_error, 0, config.evaluation.mape_clip_value)
                )
        ...
        if target_scaler_object and transform_info_dict and return_predictions:
            ...
            try:
                ...
                inverse_batch_size = min(
                    1000, n_samples
                )  # Process inverse transform in smaller batches
                ...
                if not (
                    np.isnan(predictions_original_scale).any()
                    or np.isnan(actuals_original_scale).any()
                ):
                    rmse_orig = np.sqrt(
                        mean_squared_error(actuals_original_scale, predictions_original_scale)
                    )
                    r2_orig = r2_score(actuals_original_scale, predictions_original_scale)
                    mae_orig = mean_absolute_error(
                        actuals_original_scale, predictions_original_scale
                    )
                    config = get_config()
                    mape_orig_capped = (
                        np.mean(
                            np.clip(
                                np.abs(
                                    (actuals_original_scale - predictions_original_scale)
                                    / (
                                        np.abs(actuals_original_scale)
                                        + config.evaluation.mape_epsilon
                                    )
                                ),
                                0,
                                config.evaluation.mape_clip_value,
    ...
    def _generate_evaluation_plots(self, original_preds: np.ndarray, original_actuals: np.ndarray):
        ...
        plt.hist(
            residuals_original, bins=50, alpha=0.7, color="skyblue", edgecolor="black", density=True
        )
        plt.axvline(
            mean_residuals_original,
            color="red",
            linestyle="dashed",
            linewidth=2,
        ...
        plt.text(
            0.95,
            0.90,
            f"Std Dev of Residuals: {std_residuals_original:.2f}",
    ...
    def predict(
        ...
        self._validate_sequence_input(X_batched_tensor, "X", "LSTM predict")
        ...
        with torch.inference_mode():
            for (
                inputs_batch,
            ) in loader:  # inputs_batch is already on device from _ensure_batched_input
    ...
    def plot_training_history(
        ...
        for i, (title, keys, ylog, style_specs_list) in enumerate(
            metrics_plot_config
        ):  # Renamed line_colors to style_specs_list for clarity
            ax = axes[i]
            for k_idx, key in enumerate(keys):
                if key in self.history and self.history[key]:
                    ...
                    has_marker = len(style_spec) <= 2 and any(
                        c in style_spec for c in "ovs^<>ph*H+xDd,."
                    )
                    ...
                    if "val" in key and self.history[key]:  # Ensure list is not empty
                        ...
                        if (
                            len(current_metric_values) > 0
                        ):  # Ensure there are values to find min/max
                            ...
                            if best_idx < len(self.history["epochs"]):  # Check bounds
                                ax.scatter(
                                    self.history["epochs"][best_idx],
                                    current_metric_values[best_idx],
                                    s=100,
                                    c="gold",
                                    marker="*",
                                    zorder=5,
                                    label=f'Best {label.split(" ")[0]}',
                                )  # Added space in label
        ...
        ax_ratio = axes[6]
        if (
            all(k in self.history and self.history[k] for k in ["train_loss", "val_loss"])
            and len(self.history["train_loss"]) == len(self.history["val_loss"])
            and len(self.history["train_loss"]) > 0
        ):  # Ensure lists are not empty
            train_loss_s = np.maximum(
                np.array(self.history["train_loss"]), 1e-9
            )  # Ensure train_loss_s has values
            val_loss_s = np.array(self.history["val_loss"])
            if len(train_loss_s) == len(
                val_loss_s
            ):  # Ensure equal length after potential np.array conversion
                ratio = val_loss_s / train_loss_s
                ax_ratio.plot(
                    self.history["epochs"][: len(ratio)],
                    ratio,
                    color="slateblue",
                    label="Val/Train Loss Ratio",
                )  # Explicit color, ensure epochs match ratio length
                ...
                ax_ratio.text(
                    0.5, 0.5, "Loss data length mismatch", transform=ax_ratio.transAxes, ha="center"
                )
        else:
            ax_ratio.text(
                0.5,
                0.5,
                "Loss data missing or incomplete",
                transform=ax_ratio.transAxes,
                ha="center",
    ...
    def load(cls, path: str, device: str = "cpu", strict: bool = False) -> "WeatherLSTM":
        ...
        if file_path.suffix == ".pt":
            ...
            try:
                ...
                model_params_dict = checkpoint.get(
                ...
                model.history = checkpoint.get(
                    "history", {key: [] for key in model.history_template_keys}
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    safe_globals.extend([PowerTransformer, StandardScaler, MinMaxScaler])
                except ImportError:
                    # scikit-learn might not be available
                    pass

                if hasattr(torch_serialization, "add_safe_globals"):
                    torch_serialization.add_safe_globals(safe_globals)
    ...
    def predict_with_uncertainty(
        ...
        try:
            ...
            if target_scaler is not None and transform_info is not None:
                inverse_batch_size = min(
                    10, mc_samples
                )  # Process inverse transforms in smaller batches
                ...
                for batch_start in range(0, mc_samples, inverse_batch_size):
                    ...
                    for i in range(batch_start, batch_end):
                        ...
                        predictions_original_mc[i, :, :] = inverted_sample.reshape(
                            current_batch_scaled.shape
                        )
            ...
            results = {
                "mean": np.mean(predictions_original_mc, axis=0),
            ...
            if return_samples:
                results["samples"] = predictions_original_mc
            return results
        finally:
    ...
    def plot_prediction_with_uncertainty(
        ...
        current_transform_info = (
            transform_info if transform_info is not None else self.transform_info
        )
        ...
        for i, sample_idx_in_batch in enumerate(plot_indices):
            ...
            for j in range(min(10, mc_samples)):
                ax.plot([0], [all_mc_samps[j]], "o", alpha=0.15, c="gray", ms=3)
            ax.errorbar(
                [0],
                [mean_val],
            ...
            if y_true_original_np is not None and sample_idx_in_batch < len(y_true_original_np):
                ...
                err = abs(mean_val - actual_val)
                in_ci = lower_val <= actual_val <= upper_val
                ax.text(0.02, 0.95, f"Err: {err:.2f}", transform=ax.transAxes, fontsize=9)
                ax.text(
                    0.02,
                    0.90,
                    f"In CI: {'Y' if in_ci else 'N'}",
                    transform=ax.transAxes,
                    fontsize=9,
                    color="g" if in_ci else "r",
                )

            iw = upper_val - lower_val
            ax.text(0.02, 0.85, f"IW: {iw:.2f}", transform=ax.transAxes, fontsize=9)
            ax.set_title(f"Pred for Sample Idx in Batch: {sample_idx_in_batch}", fontsize=10)

            plot_min_y_vals = [lower_val] + (
                [y_true_original_np[sample_idx_in_batch]]
                if y_true_original_np is not None and sample_idx_in_batch < len(y_true_original_np)
                else []
            )
            plot_max_y_vals = [upper_val] + (
                [y_true_original_np[sample_idx_in_batch]]
                if y_true_original_np is not None and sample_idx_in_batch < len(y_true_original_np)
                else []
            )
            ...
            ax.set_ylim(
                [max(config.data.min_radiation_clip, plot_min_y - padding), plot_max_y + padding]
            )
            ax.set_xticks([])
            ax.set_ylabel("GHI (Original Scale)", fontsize=9)
            if i == 0:
                ax.legend(fontsize=8, loc="best")

        plt.suptitle(
            f"LSTM Preds w {int((1-alpha)*100)}% CI (Original Scale)",
            fontsize=14,
            fontweight="bold",
        )
