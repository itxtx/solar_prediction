...
def prepare_weather_data(df_input, target_col, window_size=12, test_size=0.2, val_size=0.25, 
    ...
    Args:
        ...
        min_radiation_floor: Floor for radiation values AFTER original clipping but BEFORE power transform.
        clip_original_target_before_transform: Whether to clip the original target (e.g., Radiation)
    ...
    # Step 0: Apply initial min_target_threshold to original target
    ...
    power_transform_details = {'applied': False, 'type': None, 'lambda': None, 
                               ...
                               'power_transformer_obj': None, 'original_clip_bounds': None} # Added original_clip_bounds

    # Step 1: Optional clipping of the original target variable BEFORE other transforms
    # Typically applied if target_col is 'Radiation' and power transform is intended
    if clip_original_target_before_transform and target_col_processing == 'Radiation': # Or other specified targets
        if target_col_processing not in df.columns:
            ...
            # Store these bounds in power_transform_details as this clipping is often in context of preparing for power transform
            power_transform_details['original_clip_bounds'] = (float(lower_bound_orig), float(upper_bound_orig))

            print(f"Clipping original '{target_col_processing}' to range [{lower_bound_orig:.4f}, {upper_bound_orig:.4f}] "
                  f"(based on {original_clip_lower_percentile}th and {original_clip_upper_percentile}th percentiles of original scale) "
                  f"BEFORE other transformations.")
            df[target_col_processing] = np.clip(original_target_values, lower_bound_orig, upper_bound_orig)


    # Step 2. Piecewise Transform (if applicable, typically for Radiation)
    # Operates on the potentially original-clipped target_col_processing
    if target_col_processing == 'Radiation' and use_piecewise_transform: # Check against current state of target_col_processing
        print(f"Applying piecewise radiation transform to '{target_col_processing}' column.")
        df['Radiation_transformed'] = piecewise_radiation_transform(df[target_col_processing].values) # Use current state
        piecewise_transform_details = {'applied': True, 'type': 'piecewise_radiation', 'original_col': target_col_processing}
        target_col_processing = 'Radiation_transformed' # Update name
        print(f"Using '{target_col_processing}' as target for further processing.")

    # Step 3. Power Transform OR Log Transform
    # Operates on target_col_processing (which might be original, original-clipped, or piecewise-transformed)
    current_target_is_radiation_family = target_col_processing == 'Radiation' or \
                                       target_col_processing == 'Radiation_transformed' # Check current name

    if use_power_transform and current_target_is_radiation_family: # Primarily for Radiation family
        print(f"Applying Yeo-Johnson Power Transform to '{target_col_processing}'")
        
        # Apply floor AFTER original clipping but BEFORE power transform
        if min_radiation_floor is not None: # Allow min_radiation_floor = 0
        ...
        # Update existing dict, original_clip_bounds might have been set earlier
        power_transform_details.update({
            ...
            'original_col': col_fed_to_power_transform, # Name of col fed into PowerTransformer
        ...
        use_log_transform = False # Disable log if power transform was applied to the main target path

    elif use_log_transform: # Log transform logic (if power transform wasn't applied or for other targets)
        # Determine if current target_col_processing is eligible for log
        is_other_loggable = target_col_processing in ['Temperature', 'Speed'] # If not already transformed by piecewise/power
        eligible_for_log = current_target_is_radiation_family or is_other_loggable # current_target_is_radiation_family might be true if piecewise was applied
        ...
        if eligible_for_log and target_col_processing in df.columns:
            ...
            if current_target_is_radiation_family: 
            ...
            if clip_log_target: # Clipping for log-transformed values
    ...
    # Ensure all columns in cols_to_scale actually exist in df before trying to subset
    cols_to_scale = [col for col in cols_to_scale if col in df.columns]
    if target_col_actual not in cols_to_scale and target_col_actual in df.columns: # Add target if it was filtered out but exists
        cols_to_scale.append(target_col_actual)
    
    if not df[cols_to_scale].isna().all().all(): # Check if not all values are NaN before ffill/bfill
    ...
    for col in feature_cols:
        if col not in df_for_scaling.columns:
            print(f"Warning: Feature column '{col}' not found in df_for_scaling. Skipping scaling for this feature.")
            continue
        ...
        if np.isnan(values).all():
            ...
            scaled_data_df[col] = values.flatten() # Keep as NaN
        else:
    ...
    if target_col_actual not in df_for_scaling.columns:
        raise ValueError(f"Final target column '{target_col_actual}' not found in df_for_scaling for scaling.")
    ...
    if np.isnan(target_values_to_scale).all():
    ...
    if isinstance(target_scaler, StandardScaler):
        # Check if scaler has been fitted and has attributes
        if hasattr(target_scaler, 'mean_') and target_scaler.mean_ is not None:
             ...
             print(f"DEBUG [PREPARE DATA]: Target Scaler ({TargetScalerClass.__name__}) was not fitted or attributes (mean_, scale_) not found (possibly all NaN input).")


    # --- Create Sequences ---
    X, y_seq = [], [] 
    if len(scaled_data_df) <= window_size:
        # Check if scaled_data_df is empty or has all NaNs which might lead to this
        if scaled_data_df.empty or scaled_data_df.isna().all().all():
            raise ValueError(f"scaled_data_df is empty or all NaN. Data length ({len(scaled_data_df)}) is <= window_size ({window_size}). Cannot create sequences.")
        raise ValueError(f"Data length ({len(scaled_data_df)}) is <= window_size ({window_size}). Cannot create sequences.")

    # Drop rows with any NaNs in features or target before creating sequences
    # This is crucial if NaNs were introduced or not fully handled by ffill/bfill earlier
    # (e.g. if NaNs were at the very beginning of the series)
    sequence_input_df = scaled_data_df[feature_cols + [target_col_actual]].dropna()
    if len(sequence_input_df) <= window_size:
         raise ValueError(f"Data length after dropping NaNs ({len(sequence_input_df)}) is <= window_size ({window_size}). Cannot create sequences.")
    ...
    if not X: # If no sequences were created
        # This can happen if len(sequence_input_df) - window_size is 0 or negative
        print("Warning: No sequences were created. X and y will be empty. Check data length and window size after NaN handling.")
        # Define empty arrays with appropriate dimensions to prevent downstream errors
        ...
        if len(X_temp) == 0: 
            ...
            if len(X_temp) >= 2 and actual_val_size_for_split > 0 and actual_val_size_for_split < 1:
                 ...
                 val_shape_x = (0, X_train.shape[1], X_train.shape[2]) if X_train.ndim == 3 and X_train.shape[1]>0 else (0, X_train.shape[1] if X_train.ndim ==2 and X_train.shape[1]>0 else 0)
                 if X_train.ndim == 3 and X_train.shape[1] == 0 : val_shape_x = (0,0,0) # Handle case where window_size might be 0 if X_train is (n,0,m)
                 elif X_train.ndim == 2 and X_train.shape[1] == 0 : val_shape_x = (0,0)
    ...
    if power_transform_details['applied']:
        ...
        # The object is already in power_transform_details, but pop it before adding to combined_transform_info
        # to avoid serializing the object if this info dict is saved as JSON, for example.
        # Instead, we'll store it in the 'scalers' dictionary.
        ...
        combined_transform_info['transforms'].append(power_transform_details) # Now it doesn't contain the raw object
